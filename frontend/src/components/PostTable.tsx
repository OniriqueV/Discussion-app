...
export default function PostTable({
  ...
  const {
    ...
    initialSortField: "created_at",
    initialSortOrder: "desc",
    // Thêm custom filter cho company - chỉ lọc khi user chọn cụ thể
    customFilter: (post: Post) => {
      // Nếu không chọn công ty cụ thể (selectedCompanyId = undefined hoặc ""), hiển thị tất cả
      if (!selectedCompanyId) {
        return true;
      }
      // Nếu chọn công ty cụ thể, chỉ hiển thị posts của công ty đó
      return post.company_id === selectedCompanyId;
    }
  });
  ...
  useEffect(() => {
    async function fetchCompanies() {
      try {
        ...
        // KHÔNG tự động chọn công ty, để user có thể xem tất cả posts
        // Chỉ tự động chọn nếu là ca_user/member và chỉ có 1 công ty duy nhất
        if (!isAdminUser && companies.length === 1) {
          // Có thể comment dòng này nếu muốn luôn hiển thị "Tất cả" ban đầu
          // setSelectedCompanyId(companies[0].id);
        }
      } catch (e) {
        console.error('Error fetching companies:', e);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {/* Company Filter - hiển thị cho tất cả user nhưng với quyền khác nhau */}
          {companyOptions.length > 0 && (
            <select
              value={selectedCompanyId ?? ""}
              onChange={(e) => setSelectedCompanyId(e.target.value ? Number(e.target.value) : undefined)}
