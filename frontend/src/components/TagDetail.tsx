...
// import { getPosts, getPostsByTag, Post, QueryPostParams } from '@/api/postApi';
...
// const TagDetail: React.FC<TagDetailProps> = ({ tagSlug }) => {
...
//         // Fetch actual tag data from API
//         const tagResponse = await tagApi.findBySlug(tagSlug);
//         if (tagResponse.success && tagResponse.data) {
//           setTag(tagResponse.data);
//         } else {
//           // If tag not found in API, create a mock tag for fallback
...
//         // Load posts with tag filter using the new API method
//         const postsResponse = await getPostsByTag(tagSlug, { 
//           page: 1,
//           limit: itemsPerPage,
...
//         // Create fallback tag if API fails
//         const fallbackTag: TagType = {
//           id: tagSlug,
//           name: tagSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
//           slug: tagSlug,
//           created_at: new Date().toISOString(),
...
//       loadTagData();
//     }
//   }, [tagSlug, filters.sortBy, filters.sortOrder]);
...
//         const params: Omit<QueryPostParams, 'tag'> = {
//           page: 1,
//           limit: itemsPerPage,
//           sort_by: filters.sortBy,
//           sort_order: filters.sortOrder
...
//         const response = await getPostsByTag(tagSlug, params);
//         if (response.data) {
...
//   }, [searchTerm, filters, tag, tagSlug]);
...
//   // Calculate statistics from actual posts data
//   const resolvedPosts = filteredPosts.filter(post => post.status === "solve").length;
//   const unresolvedPosts = filteredPosts.filter(post => post.status === "problem").length;
...
//             <h1 className="text-2xl font-bold text-gray-900 mb-2">Không tìm thấy thẻ</h1>
//             <p className="text-gray-600 mb-4">Thẻ bạn tìm kiếm không tồn tại.</p>
//             <button
//               onClick={() => router.back()}
...
//             <span className="text-gray-900 font-medium">Thẻ: {tag.name}</span>
//           </nav>
//         </div>
...
//                   Khám phá tất cả bài viết với thẻ {tag.name}
//                 </p>
//                 <p className="text-purple-200 text-sm mt-1">
//                   Được tạo vào {new Date(tag.created_at).toLocaleDateString('vi-VN')}
//                 </p>
//                 {tag.postCount > 0 && (
//                   <p className="text-purple-200 text-sm">
//                     {tag.postCount} bài viết được gắn thẻ này
//                   </p>
//                 )}
//               </div>
//             </div>
...
//               <div className="ml-5 w-0 flex-1">
//                 <dl>
//                   <dt className="text-sm font-medium text-gray-500 truncate">Tổng lượt xem</dt>
...
//                 Hiển thị <span className="font-medium text-purple-600">{pagination.total}</span> kết quả
